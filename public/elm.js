...
Elm.Components.make = function (_elm) {
   ...
   var handleField = F2(function (content,address) {
      ...
      _U.list([$Html$Attributes.$class("field")
              ,$Html$Attributes.placeholder("type their handle, then press enter")
              ,$Html$Attributes.value(content)
              ,A2($Html$Events.onKeyDown,address,function (i) {    return $Types.HandleInput(i);})
              ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (str) {    return A2($Signal.message,address,$Types.RefreshField(str));})]),
      _U.list([]));
   });
   var tf$header = A2($Html.p,
   _U.list([$Html$Attributes.$class("point"),$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "margin-top",_1: "4px"}]))]),
   _U.list([$Html.text("Their Feed")]));
...
Elm.Init = Elm.Init || {};
Elm.Init.make = function (_elm) {
   "use strict";
   _elm.Init = _elm.Init || {};
   if (_elm.Init.values) return _elm.Init.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Types = Elm.Types.make(_elm);
   var _op = {};
   var initialTweets = _U.list([A5($Types.Tweet,
...
Elm.Main.make = function (_elm) {
   ...
   var update = F2(function (action,m) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Search": return {ctor: "_Tuple2",_0: m,_1: $Effects.none};
   ...
   return _elm.Main.values = {_op: _op,init: init,search: search,update: update,view: view,app: app,main: main};
};
